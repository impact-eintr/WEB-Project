...
func BridgeQuery(count int) string {
	...
	rows, err := global.DB.Query("select `桥梁名称`, `桥梁代码`, `桥梁中心桩号`, `路线编号`, `路线名称`, `技术等级`, `桥梁全长(米)`, `跨径总长(米)`, `单孔最大跨径(米)`, `跨径组合(孔*米)`, `桥梁全宽(米)`, `桥面净宽(米)`, `按跨径分类代码`, `按跨径分类类型` from l24 where `技术等级`=? AND `ID`>2", level)
	if err != nil {
		log.Println(err)
		return ""
	}
